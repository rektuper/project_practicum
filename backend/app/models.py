...
class WorkEventParticipant(Base):
    ...
    id = Column(Integer, primary_key=True, index=True, autoincrement=True)
    work_event_id = Column(String, ForeignKey("work_events.id", ondelete="CASCADE"), nullable=False, index=True)
    employee_id = Column(String, ForeignKey("employees.id", ondelete="CASCADE"), nullable=False, index=True)
...
class Task(Base):
    ...
    author_id = Column(String, ForeignKey("employees.id"), nullable=False, index=True)
